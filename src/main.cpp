...
void setup()
{
    ...
    LOG_DEBUG("screenWidth = %d, screenHeight = %d\n", screenWidth, screenHeight);
#ifdef DIRECT_MODE
    ...
    if (!disp_draw_buf)
    {
        LOG_ERROR("LVGL disp_draw_buf allocate failed!");
    }
    else
    {
        ...
        indev_drv.read_cb = my_touchpad_read; // add touch cb
        lv_indev_drv_register(&indev_drv);

        device_data_init();
        ui_init();
        LOG_DEBUG("UI init done\n");
    }
}
